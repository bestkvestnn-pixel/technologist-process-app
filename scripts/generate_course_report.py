...
Примечание: фактическая разработка выполнена в Codex. Методика требует сравнение 2-3 LLM; результаты других LLM не выдумываются и отмечены как требующие отдельного фактического прогона.
"""
...
def build():
    ...
    story.extend(
        [
            ...
            p("Проверка соответствия методическим указаниям:", styles["BodyNoIndent"]),
            table(
                [
                    ["Требование методички", "Как выполнено в отчёте"],
                    ["Объём 4-7 страниц", "PDF сформирован на 7 страницах A4."],
                    ["Структура работы", "Есть титульный лист, введение, задача, LLM, ход работы, анализ, выводы, источники."],
                    ["Сравнительная таблица", "Добавлена таблица по параметрам методички; неиспользованные LLM отмечены честно."],
                    ["Оформление", "Arial 12 пт, интервал 1,5, поля 30/15/20/20 мм, нумерация страниц."],
                    ["Исходные файлы", "Код, генератор отчёта, PDF и markdown находятся в репозитории GitHub."],
                ],
                [54 * mm, 108 * mm],
                styles,
            ),
    ...
    story.extend(
        [
            ...
            p(
                "Методические указания требуют выполнить задачу с помощью 2-3 различных LLM/инструментов. В данном отчете полностью заполнен фактический результат для Codex. Результаты DeepSeek, GigaChat, Claude, Cursor или GitHub Copilot не подставлялись искусственно: для строгого академического сравнения их нужно получить отдельными фактическими прогонами по тому же техническому заданию.",
                styles["BodyRus"],
            ),
            table(
                [
            ...
            p("Ключевые промпты и действия:", styles["Heading2Rus"]),
            table(
                [
                    ["Итерация", "Запрос пользователя", "Результат"],
                    ["1", "Сделать приложение для разработки ТП", "Создан базовый интерфейс и доменная логика."],
                    ["2", "Добавить G-код для NC 210 и Sinumerik", "Реализован модуль cnc.js."],
                    ["3", "Сформировать отчёт и затем ТП", "Добавлен отчёт по исходнику и документы ГОСТ."],
                    ["4", "Подбирать инструмент по каталогам", "Добавлен tool-catalog.js и оптимальные диаметры."],
                    ["5", "Сделать стартовую страницу и GitHub Pages", "Добавлен стартовый экран и опубликован сайт."],
                ],
                [18 * mm, 60 * mm, 84 * mm],
                styles,
            ),
            ...
            table(
                [
                    ...
                    [
                        ...
                        "не фиксировалось таймером",
                        "не оценивалось",
                        "не оценивалось",
                        "Точное время не фиксировалось секундомером, поэтому не подменяется оценкой.",
                    ],
                    [
                        "Финансовые затраты",
                        "0 ₽ прямых API-затрат в рамках локального сеанса",
                        "не оценивалось",
                        "не оценивалось",
                        "Подписочные расходы не рассчитывались как токены API.",
                    ],
                    [
    ...
    story.extend(
        [
            ...
            p(
                "Методическое ограничение отчета: фактический второй и третий прогон в других LLM не выполнялись в рамках текущего проекта. Чтобы полностью закрыть сравнительный раздел без допущений, необходимо повторить одинаковое ТЗ в двух дополнительных инструментах и внести реальные метрики в таблицу анализа.",
                styles["BodyRus"],
            ),
